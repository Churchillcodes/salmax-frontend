...
import { ChevronLeft, ShoppingBag, Truck, Calendar, Tag, Layers, RefreshCw } from 'lucide-react';
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      ...
      try {
        ...
        const initialImage = response.data.image || imagesList[0] || '';
        setActiveImage(initialImage);

        // Fetch related products (same category)
        try {
          const allProductsRes = await apiClient.get('/products');
          const catId = typeof response.data.category === 'object' ? response.data.category._id || response.data.category.id : response.data.category;
          
          const filtered = (allProductsRes.data || [])
            .filter(p => p.isActive !== false && (p._id || p.id) !== id && 
              (typeof p.category === 'object' ? p.category._id || p.category.id : p.category) === catId
            )
            .slice(0, 4);
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] bg-dark-base flex flex-col items-center justify-center text-warm-ivory">
        ...
        <p className="text-xs uppercase tracking-widest text-gold/80 animate-pulse">Loading Collection Item...</p>
      </div>
    );
  }
  ...
  // Evaluate total stock levels
  const hasSizes = product.sizes && Object.keys(product.sizes).length > 0;
  const isOutOfStock = product.stock === 0 || (hasSizes && Object.values(product.sizes).every(qty => qty === 0));
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="aspect-[4/5] bg-dark-charcoal/50 border border-gold/10 rounded-xl overflow-hidden relative flex items-center justify-center">
              {activeImage ? (
                ...
                <span className="font-serif italic text-gold/30">Collection Item</span>
              )}
              {isOutOfStock && (
                ...
                {images.map((imgUrl, index) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-lg overflow-hidden border shrink-0 premium-transition bg-dark-charcoal/50 ${
                      activeImage === imgUrl ? 'border-gold' : 'border-gold/10 hover:border-gold/40'
                    }`}
                  >
          ...
          <div className="lg:col-span-5 flex flex-col justify-between">
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2 items-center text-xs tracking-widest uppercase">
                ...
                <span className="text-warm-ivory/50">{product.productType || 'Curated Supply'}</span>
                {product.group && (
                  <>
                    <span className="text-white/20">&bull;</span>
              ...
              <div className="font-serif text-2xl text-gold border-b border-gold/10 pb-5">
                {product.price ? `KES ${product.price.toLocaleString()}` : 'Price on request'}
              </div>

              {/* Description */}
              <div className="space-y-3 text-sm font-light text-warm-ivory/70 leading-relaxed">
                <h3 className="text-xs uppercase tracking-widest text-white/40 font-semibold font-sans">Description</h3>
                <p>{product.description || 'No description provided for this boutique collection item.'}</p>
              </div>

              {/* Size & Stock Information */}
              <div className="space-y-4 pt-4 border-t border-gold/5">
                <h3 className="text-xs uppercase tracking-widest text-white/40 font-semibold">Available Sizes & Stock</h3>
              ...
              <div className="grid grid-cols-2 gap-4 pt-6 border-t border-gold/5 text-xs font-light text-warm-ivory/50">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <h4 className="font-semibold text-white/80">Secured Dispatch</h4>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <h4 className="font-semibold text-white/80">Tailored Orders</h4>
