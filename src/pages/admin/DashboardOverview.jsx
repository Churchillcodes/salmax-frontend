    ...
    const fetchDashboardStats = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "Primary /dashboard/summary endpoint failed, calculating client-side...",
          );
        }
        ...
        const leads = (leadsRes.data || []).map((lead) => normalizeLead(lead));
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {productCards.map((card) => {
            ...
            return (
              <div
                key={card.label}
                className="bg-dark-charcoal border border-gold/10 p-5 rounded-xl flex items-center justify-between"
              >
                ...
                <div
                  className={`w-12 h-12 rounded-lg ${card.bg} ${card.color} border border-gold/5 flex items-center justify-center`}
                >
                  <Icon size={20} />
                </div>
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {orderCards.map((card) => {
            ...
            return (
              <div
                key={card.label}
                className="bg-dark-charcoal border border-gold/10 p-4 rounded-xl flex flex-col justify-between h-28"
              >
                <div
                  className={`w-8 h-8 rounded-lg ${card.bg} ${card.color} border border-gold/5 flex items-center justify-center self-end`}
                >
                  <Icon size={16} />
                </div>
                <div className="mt-2">
                  <p className="text-[10px] text-warm-ivory/50 font-light uppercase tracking-wider">
                    {card.label}
                  </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-dark-charcoal border border-gold/10 rounded-xl p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link
              to="/admin/orders"
              className="text-xs text-gold hover:text-gold-light transition duration-300 flex items-center gap-1"
            >
              All Orders
              <ArrowRight size={12} />
            </Link>
          ...
          {recentOrders.length > 0 ? (
            <div className="divide-y divide-gold/5">
              {recentOrders.map((order) => (
                <div
                  ...
                  className="py-3.5 flex justify-between items-center text-sm font-light"
                >
                  <div>
                    <p className="font-semibold text-white">
                      Ref: #{order._id?.slice(-6) || order.id}
                    </p>
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={`inline-block text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-full mt-1 ${
            ...
            <p className="text-xs text-warm-ivory/40 text-center py-8">
            ...
            <div className="divide-y divide-gold/5">
              {recentLeads.map((lead) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <p
                      className="text-xs text-gold font-medium truncate max-w-[150px]"
