...
export default function SalesAnalytics() {
  ...
  const [summary, setSummary] = useState({ totalRevenue: 0, averageOrder: 0, totalOrdersCount: 0 });
    ...
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn("Backend analytics aggregates failed or pending, compiling fallback datasets...");
        }
        ...
        const productsRes = await apiClient.get('/products').catch(() => ({ data: [] }));
        const products = productsRes.data || [];

        // Compile Summary Card Values
        const deliveredOrders = orders.filter(o => o.status?.toLowerCase() === 'delivered');
        const revenueOrders = orders.filter(o => o.status?.toLowerCase() !== 'cancelled');
        
        const totalRevenue = revenueOrders.reduce((acc, curr) => acc + (curr.totalAmount || curr.total || 0), 0);
        const averageOrder = revenueOrders.length > 0 ? (totalRevenue / revenueOrders.length) : 0;
        ...
        if (revenueDataFetched && Array.isArray(revenueDataFetched)) {
          ...
          orders.forEach(order => {
            ...
            if (dateStr && order.status?.toLowerCase() !== 'cancelled') {
              ...
              trendsMap[formattedDate] = (trendsMap[formattedDate] || 0) + (order.totalAmount || order.total || 0);
            }
          });
          const trendsArray = Object.entries(trendsMap).map(([date, revenue]) => ({ date, revenue })).slice(-7);
          ...
          orders.forEach(order => {
            if (order.status?.toLowerCase() !== 'cancelled' && order.items) {
              order.items.forEach(item => {
                ...
                typeSalesMap[type] = (typeSalesMap[type] || 0) + (item.price * (item.quantity || 1) || 0);
              });
            }
          });
          const typeBreakdown = Object.entries(typeSalesMap).map(([name, value]) => ({ name, value }));
  ...
  const handleCustomerSearch = (e) => {
    ...
    if (customerOrders.length > 0) {
      const name = customerOrders[0].customerName || customerOrders[0].customer?.name;
      const phone = customerOrders[0].customerPhone || customerOrders[0].customer?.phone;
      const email = customerOrders[0].customerEmail || customerOrders[0].customer?.email;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-gold">
        ...
        <p className="text-xs uppercase tracking-widest animate-pulse">Plotting Charts & Aggregates...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10 animate-fade-in text-warm-ivory">
      ...
      {/* Title */}
      <div>
        <h1 className="font-serif text-2xl md:text-3xl text-white font-light tracking-wide">Sales Analytics</h1>
        <p className="text-xs text-warm-ivory/50 mt-1 uppercase tracking-widest">
          Revenue curves, customer purchase logs, and top-selling collections.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-dark-charcoal border border-gold/10 p-5 rounded-xl flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] text-warm-ivory/40 uppercase tracking-wider font-light">Gross Sales Revenue</p>
        ...
        <div className="bg-dark-charcoal border border-gold/10 p-5 rounded-xl flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] text-warm-ivory/40 uppercase tracking-wider font-light">Average Order Value</p>
            <h3 className="font-serif text-xl font-bold text-white mt-0.5">
        ...
        <div className="bg-dark-charcoal border border-gold/10 p-5 rounded-xl flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] text-warm-ivory/40 uppercase tracking-wider font-light">Total Logged Orders</p>
            <h3 className="font-serif text-xl font-bold text-white mt-0.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-dark-charcoal border border-gold/10 rounded-xl p-6">
          <h3 className="font-serif text-base text-white tracking-wide mb-6">Revenue Growth curve</h3>
          <div className="h-72">
            {revenueData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={revenueData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#1A1A1A', borderColor: '#C8A24A', color: '#F7F3EA' }}
                    formatter={(val) => [`KES ${val.toLocaleString()}`, 'Revenue']}
                  />
                  <Area type="monotone" dataKey="revenue" stroke="#C8A24A" strokeWidth={2} fillOpacity={1} fill="url(#colorRevenue)" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <p className="text-center py-20 text-xs text-warm-ivory/40">No sales transactions available to graph.</p>
            )}
          </div>
        </div>

        {/* Product Type Breakdown Bar Chart */}
        <div className="bg-dark-charcoal border border-gold/10 rounded-xl p-6">
          <h3 className="font-serif text-base text-white tracking-wide mb-6">Sales Value by Product Type</h3>
          <div className="h-72">
            {breakdown.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={breakdown}>
                  ...
                  />
                  <Bar dataKey="value" fill="#C8A24A" radius={[4, 4, 0, 0]}>
              ...
              <p className="text-center py-20 text-xs text-warm-ivory/40">No product classifications cataloged.</p>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* Top selling products (Columns 1-5) */}
        <div className="lg:col-span-5 bg-dark-charcoal border border-gold/10 rounded-xl p-6 flex flex-col justify-between">
          <div>
            <h3 className="font-serif text-base text-white tracking-wide mb-6">Top-Selling Catalogue Items</h3>
            {topProducts.length > 0 ? (
              <div className="space-y-4">
                {topProducts.map((prod, idx) => (
                  <div key={prod.name} className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-3">
                      <div className="w-6 h-6 rounded-full bg-gold/10 border border-gold/20 flex items-center justify-center text-gold font-serif text-[10px]">
                        {idx + 1}
                      </div>
                      <span className="font-semibold text-white/90 truncate max-w-[160px]">{prod.name}</span>
                    </div>
                    <span className="text-gold/90 font-medium font-serif bg-gold/5 border border-gold/10 px-2 py-0.5 rounded">
                      {prod.sales} Sold
                    </span>
                  </div>
                ))}
              ...
              <p className="text-xs text-warm-ivory/40 text-center py-10">No top products data compiled.</p>
            )}
          </div>
          <div className="flex gap-2.5 p-3.5 bg-gold/5 border border-gold/10 rounded-lg text-[10px] leading-relaxed text-gold/90 font-light mt-6">
            ...
            <p>Highlights are sorted based on cumulative product counts inside valid sales invoices.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-dark-charcoal border border-gold/10 rounded-xl p-6 space-y-6">
          <div>
            <h3 className="font-serif text-base text-white tracking-wide">Client History Lookup</h3>
          ...
          <form onSubmit={handleCustomerSearch} className="flex gap-3">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gold/50" size={14} />
              <input
                type="text"
          ...
          {customerHistory && (
            <div className="border border-gold/15 rounded-lg p-4 bg-dark-base/30 text-xs font-light space-y-4 animate-fade-in">
              {customerHistory.error ? (
                <p className="text-center text-amber-500 font-medium py-4">{customerHistory.error}</p>
              ) : (
                <>
                  {/* Bio */}
                  <div className="grid grid-cols-2 gap-4 border-b border-gold/5 pb-3">
                    <div>
                      <h4 className="font-semibold text-white/50 uppercase tracking-widest text-[9px] mb-1">Customer Profile</h4>
                      <p className="text-sm font-semibold text-white">{customerHistory.name}</p>
                      <p className="mt-0.5 text-warm-ivory/60">{customerHistory.phone}</p>
                      <p className="text-warm-ivory/60">{customerHistory.email}</p>
                    </div>
                    <div className="text-right">
                      <h4 className="font-semibold text-white/50 uppercase tracking-widest text-[9px] mb-1">Lifetime Value</h4>
                      <p className="text-lg font-bold text-gold font-serif mt-1">
                        KES {customerHistory.totalSpent.toLocaleString()}
                      </p>
                      <p className="text-[10px] text-warm-ivory/40 uppercase mt-0.5">{customerHistory.orders.length} order(s)</p>
                    </div>
                  </div>

                  {/* Orders list */}
                  <div className="space-y-2">
                    <h5 className="font-semibold text-white/50 uppercase tracking-wider text-[9px]">Purchases History</h5>
                    <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                      {customerHistory.orders.map((ord) => (
                        <div key={ord._id || ord.id} className="bg-dark-base/40 p-2.5 rounded border border-gold/5 flex justify-between items-center text-[11px]">
                          <div>
                            <span className="font-semibold text-white">#{ (ord._id || ord.id).slice(-6).toUpperCase() }</span>
                            <span className="text-warm-ivory/40 ml-2">
                              {ord.createdAt || ord.date ? new Date(ord.createdAt || ord.date).toLocaleDateString() : ''}
                            </span>
                          </div>
                          <div className="text-right font-serif">
                            <span className="text-gold font-semibold mr-3">
                              KES {(ord.totalAmount || ord.total || 0).toLocaleString()}
                            </span>
                            <span className="text-[8px] uppercase tracking-wider text-warm-ivory/50">{ord.status}</span>
                          </div>
                        </div>
                      ))}
                    </div>
